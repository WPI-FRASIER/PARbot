...
int main(int argc, char** argv){
  ...
  while (node.ok()){
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_depth_frame", "base_link"));
    /*transform.setOrigin( tf::Vector3(-0.1, 0.0, -1.0) );
    transform.setRotation( tf::Quaternion(0, 0, 0) );
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "primsense1_depth_frame", "base_link"));
    transform.setOrigin( tf::Vector3(0.0, 0.0, -1.0) );
    transform.setRotation( tf::Quaternion(0, 0, 0) );
