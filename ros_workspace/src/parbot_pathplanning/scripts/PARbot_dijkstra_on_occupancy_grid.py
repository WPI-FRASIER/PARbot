...
def convert_occupancy_gird(grid):
    ...
    prev_row = []
    current_row = []
    queue = Queue() #returned list of GraphNodes
    ...
    for y in xrange(0, height): #make grid from top left
        print "y" , y
        num_node = 0 #numbrer of nodes made in it this y iteration
        for x in xrange(0, width):
            ...
            if grid.data[current_data] < THRESHOLD and grid.data[current_data] != -1: #check if is occupied and known
                ...
                for other in prev_row: #check for neighbors in prev_row
                    is_neighbor(node, other, res)
                if num_node != 0: #check if cell to left is neighbor
                    is_neighbor(node, current_row[num_node - 1], res)
                current_row.append(node)
                num_node += 1 #increase number of nodes made in this row

        graphNodes.extend(prev_row)
        if y == height-1: #check for terminal row
            graphNodes.extend(current_row)
        else:
            prev_row = []
            prev_row = current_row[:] #make a copy of current row and set it to prev_row
            current_row = [] #empty current roq
