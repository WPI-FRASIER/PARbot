...
public:
	...
	void transform_pose()
	{
		...
		old_pose.header.frame_id = std::string("base_footprint");
		new_pose.header.frame_id = std::string(TARGET_FRAME);

		old_pose.pose.position.x = 0;
		old_pose.pose.position.y = 0;
		old_pose.pose.position.z = 0;

		old_pose.pose.orientation.x = 0;
		old_pose.pose.orientation.y = 0;
		old_pose.pose.orientation.z = 0;
		old_pose.pose.orientation.w = 1;
