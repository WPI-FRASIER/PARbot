...
#define DELTA 0.0001
#define THRESHOLD 10000 //determines if cell is occupied before creating node
#define COST_WEIGHT 0.25

parbot_motion_planning::cost_map_2 localSavedMap;
...
public:
    ...
    void expand(GraphNode* node, GraphNode* target){
        double cost_to_node, score, total_cost;
        BOOST_FOREACH(GraphNode* n, node->neighbors)
        {
            cost_to_node = node->gn + straightLineDistance(node, n); //cost to get to where I am
            score = straightLineDistance(n, target) + cost_to_node; //Cost to get from where I am to the target
            total_cost = (1-COST_WEIGHT) * score + (COST_WEIGHT * n->cost); //Include cost map value in score
...
bool aStar(GraphNode* start, GraphNode* target){
    ...
    start->fn = (1-COST_WEIGHT)*straightLineDistance(start, target) + (COST_WEIGHT * start->cost);
    fringe.add(start);
 ...
 */
Queue convert_occupancy_grid(parbot_motion_planning::cost_map_2 map)
{
    vector<GraphNode*> prev_row;
    vector<GraphNode*> current_row;
    Queue occupancy_queue;

    float res = map.grid_size; //meters per cell
    float width = map.row_length;
    float height = map.num_rows;
    int x_offset = map.x_offset;
    int y_offset = map.y_offset;
		int test = 0;
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            double current_cost = map.column[x].row[y];
            if ((current_cost != -1)) //(current_cost < THRESHOLD) &&
            {
                //GraphNode* node = new GraphNode(res*x, res*y); //from map server
                GraphNode* node = new GraphNode((res*x - (res*x_offset)), (res*y - (res*y_offset))); //from cost map
                node->cost = current_cost;
                occupancy_queue.add(node); //add GraphNode* to queue for A*
...
void mapCallback(const parbot_motion_planning::cost_map_2 msg)
{
    ...
    GraphNode* start = get_node_from_point(localSavedMap.grid_size, start_point, generatedQ); //Find Start in Queue
    cout<<"Found Start: x= " << start->x << ", y= " << start->y <<"\n";

    GraphNode* target = get_node_from_point(localSavedMap.grid_size, end_point, generatedQ); //Find Target in Queue
...
int main(int argc, char **argv)
{
    ...
    //Subscribers
    ros::Subscriber Map_sub = n.subscribe("cost_map_2", 1, mapCallback); //map
    ros::Subscriber SlamPose_sub = n.subscribe("robot_pose", 1, robotPoseCallback); //robot position
    ros::Subscriber TargetPose_sub = n.subscribe("Tracking/object_position", 1, targetPoseCallback); //target location
